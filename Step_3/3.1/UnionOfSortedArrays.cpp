...
vi findUnion(vi arr1, vi arr2, int n, int m){
    int i = 0, j = 0;
    vi Union ;
    while (i<n and j<m){
        if(arr1[i]<=arr2[j]){
            if(Union.size()==0 or Union.back() != arr1[i]) Union.pb(arr1[i]);
            i++;
        }else if(arr1[i] > arr2[j]) {
            if(Union.size()==0 or Union.back() != arr2[j]) Union.pb(arr2[j]);
            j++;
        }
    }
    while(i<n){
        if(Union.back() != arr1[i]) Union.pb(arr1[i]);
        i++;
    }
    while(j<m){
        if(Union.back() != arr2[j]) Union.pb(arr2[j]);
        j++;
    }
    return Union;
}
...
int main() {
    ...
    vi Union = findUnion(arr1, arr2, n,m);
